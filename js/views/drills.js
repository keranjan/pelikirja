...
/**
 * Piirtovärin kuvakkeen tyyli. Kuvake saa saman reunuksen kuin kentälle
 * piirretty veto, jotta musta erottuu myös tummassa teemassa ja valkoinen
 * vaaleassa.
 */
function inkStyle(id) {
  const c = drawColor(id);
  return `color:${c.value};text-shadow:0 0 2px ${c.halo}, 0 0 2px ${c.halo}, 0 0 4px ${c.halo}`;
}
    ...
    const draw = () => {
      ...
      for (const shape of Object.values(SHAPES)) {
        body.append(pressable(h('button', { class: `list-item${tool === shape.id ? ' sel' : ''}` },
          h('span', { class: 'markicon', style: inkStyle(inkColor) },
...
export function drillView(id) {
  ...
  const tools = h('div', { class: 'toolgrid' },
    ...
    }, h('span', { class: 'gl', style: inkStyle(inkColor),
